...
// MARK: - 底部几个功能：开启闪光灯、相册、我的二维码
    ...
    func drawBottomItems() {
        ...
        btnFlash.setImage(UIImage(named: "CodeScan.bundle/qrcode_scan_btn_flash_nor"), for:.normal)
        btnFlash.addTarget(self, action: #selector(QQScanViewController.openOrCloseFlash), for: .touchUpInside)
        ...
        btnPhoto.setImage(UIImage(named: "CodeScan.bundle/qrcode_scan_btn_photo_nor"), for: .normal)
        btnPhoto.setImage(UIImage(named: "CodeScan.bundle/qrcode_scan_btn_photo_down"), for: .highlighted)
//        btnPhoto.addTarget(self, action: Selector(("openPhotoAlbum")), for: UIControlEvents.touchUpInside)

        btnPhoto.addTarget(self, action: #selector(QQScanViewController.openLocalPhotoAlbum), for: .touchUpInside)
        ...
        btnMyQR.setImage(UIImage(named: "CodeScan.bundle/qrcode_scan_btn_myqrcode_nor"), for: .normal)
        btnMyQR.setImage(UIImage(named: "CodeScan.bundle/qrcode_scan_btn_myqrcode_down"), for: .highlighted)
        btnMyQR.addTarget(self, action: #selector(QQScanViewController.myCode), for: .touchUpInside)
    ...
    {
        let alertController = UIAlertController(title: "title", message:"使用首页功能", preferredStyle: .alert)
        
        let alertAction = UIAlertAction(title:  "知道了", style: .default) { (alertAction) -> Void in
    ...
    @objc func openOrCloseFlash() {
        ...
        if isOpenedFlash
        {
            btnFlash.setImage(UIImage(named: "CodeScan.bundle/qrcode_scan_btn_flash_down"), for:.normal)
        }
        else
        {
            btnFlash.setImage(UIImage(named: "CodeScan.bundle/qrcode_scan_btn_flash_nor"), for:.normal)
